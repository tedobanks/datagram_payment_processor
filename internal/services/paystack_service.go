...
// 	"fmt"
// 	"log"
// 	"strings" // Added for error checking example
...
// // InitializePayment initializes a payment transaction with Paystack
// func (s *PaystackService) InitializePayment(req models.PaystackInitializeRequest, userID string, callbackURL string) (*paystack.TransactionInitializeResponse, error) {
// 	// Amount is in kobo for NGN. Ensure your req.Amount is already in this smallest unit.
// 	// You can add metadata to link the transaction back to your user or other entities
// 	metadata := map[string]interface{}{
// 		"user_id": userID, // CRITICAL for linking payment back to user
...
// 	// The rpip/paystack-go library expects amount to be float64
// 	transactionReq := &paystack.TransactionRequest{
// 		Email:       req.Email,
// 		Amount:      float64(req.Amount), // Amount in Kobo
// 		Currency:    "NGN",               // Or get from config/request
// 		Metadata:    metadata,
// 		CallbackURL: callbackURL, // Important for redirecting user after payment attempt
// 		//Channels: []string{"card", "bank_transfer"}, // Optionally specify payment channels
// 	}
...
// 	// Paystack's `Transaction.Initialize` might return a successful HTTP response
// 	// but with `Status: false` in the JSON body if Paystack itself couldn't initialize.
// 	if !resp.Status {
// 		log.Printf("Paystack initialization failed on Paystack's end. Message: %s, Response: %+v", resp.Message, resp)
...
// 		Amount:    float64(datacreditToWithdrawKobo), // Amount in Kobo
...
// 	// Check the status in the response body from Paystack
// 	if !transferResponse.Status {
// 		log.Printf("Paystack transfer initiation failed on Paystack's end. Message: %s, Response: %+v", transferResponse.Message, transferResponse)
// 		// The transferResponse.Message might contain useful info like "Insufficient balance" from Paystack's side.
// 		return "", fmt.Errorf("Paystack transfer initiation failed: %s", transferResponse.Message)
// 	}

// 	// transferResponse.Data should contain details of the initiated transfer, including a transfer_code
// 	transferCode := transferResponse.Data.TransferCode
// 	log.Printf("Paystack transfer successfully initiated. Transfer Code: %s, Status: %s", transferCode, transferResponse.Data.Status)
